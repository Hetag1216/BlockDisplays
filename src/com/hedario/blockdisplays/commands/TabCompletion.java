...
public class TabCompletion implements TabCompleter {
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
		if (args.length == 0 || args.length == 1) {
			...
			for (final BDCommand cmd : BDCommand.instances.values()) {
				if (Arrays.asList(cmd.getAliases()).contains(args[0].toLowerCase()) && sender.hasPermission("blockdisplays.command." + cmd.getName())) {
	...
	public static List<String> getCommandsForUser(CommandSender sender) {
		...
		for (final String command : BDCommand.instances.keySet()) {
			if (sender.hasPermission("blockdisplays.command." + command.toLowerCase())) {
